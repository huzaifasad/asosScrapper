...
export default function ScraperPage() {
  ...
  const connectWebSocket = () => {
    try {
      wsRef.current = new WebSocket('ws://13.53.190.231:4000')
  ...
  const fetchCategories = async () => {
    try {
      const response = await fetch("http://13.53.190.231:4000/categories")
  ...
  const fetchProductsFromDB = async () => {
    ...
    const { count } = await supabase.from("zara_cloth_test").select("*", { count: "exact", head: true })
  ...
  const handleScrape = async () => {
    ...
    try {
      ...
      if (scrapingMode === "search") {
        endpoint = "http://13.53.190.231:4000/scrape"
        body = {
          ...requestBody,
          searchTerm: searchTerm.trim()
        }
      } else {
        endpoint = "http://13.53.190.231:4000/scrape-category"
        body = {
          ...requestBody,
